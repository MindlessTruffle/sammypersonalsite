...
document.addEventListener('click',event=>{
 ...
 if(portal?.open&&link.classList.contains('back-link')&&new URL(link.href).pathname==='/'){event.preventDefault();requestClose();return;}
 const url=tearDestination(link.href,location.href,{modified:event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey,download:link.hasAttribute('download'),native:link.hasAttribute('data-native-navigation'),target:link.target});
